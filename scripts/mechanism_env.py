...
- `CARGO_TARGET_DIR`: the parity tests run docs example binaries from
  docs/examples/target; a redirected build would leave stale ones there.
- `REPORT_ONLY`: turns off visual_parity's regression assertion.
- `INSTA_*`: can force-pass or rewrite snapshots; `INSTA_UPDATE=no` makes a
  mismatch fail without writing anything.
...
PINNED = {"COLORTERM": "truecolor", "INSTA_UPDATE": "no"}
