...
| An app froze at startup in any terminal that answers the DECRQM mode queries for synchronized output (2026) or in-band resize (2048): nothing drew and keys were swallowed until one happened to be `u` or `c`. | Exists (fixed on Linux 2026-09-25) | Found by the inline-mode harness, which answers those queries; `pty_parity` sets `TEXTUAL_SYNC_OUTPUT=0` and answers nothing, so it never saw this. `strace` of `calculator`: the reply `ESC [ ? 2026 ; 2 $ y` is read by `crossterm::event::poll` (`src/driver/live.rs` at `2a96eb39`), then crossterm blocks in a second `read(0)`. crossterm 0.28.1 parses `CSI ?` sequences ending in anything but `u` or `c` as incomplete (`src/event/sys/unix/parse.rs:180-184`), so every later byte joins that sequence. Fix: on Linux the driver writes the queries and a DA1 query in one go and reads stdin itself, after `poll(2)` readiness, until the DA1 reply ends; other Unix systems skip the queries. Test `tests/terminal_queries.rs` (fails before the fix for answered and late replies). A terminal that answers later than 200 ms can still lose input this way |
